  ...
  <div class="edit">
    <el-form class="form" :model="form" label-width="80px">
      ...
      <el-form-item label="标签形式">
        <el-select v-model="form.label.type" placeholder="请选择标签形式" style="width: 100px">
          <el-option label="内部" value="inner"></el-option>
          <el-option label="外部" value="outer"></el-option>
          <el-option label="蜘蛛腿" value="spider"></el-option>
        </el-select>
      </el-form-item>
    </el-form>
  </div>
...
export default {
  ...
  computed: {
    options() {
      const typeLabel = this.chartObj.dataHead[1]
      const res = []
